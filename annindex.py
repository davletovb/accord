...
def build_index():
    a = AnnoyIndex(512, 'angular')
    index_map = {}
    with os.scandir('vectors/') as entries:
        for i, entry in enumerate(entries):
            if (entry.name != "index.ann") and (entry.name != "index_map.json"):
                vec = np.load('vectors/' + entry.name)
                a.add_item(i, vec[0][0])
                index_map[i] = entry.name

    with open('vectors/index_map.json', 'w') as file:
        json.dump(index_map, file, sort_keys=True, indent=4)

    a.build(10)
    a.save('vectors/index.ann')
    print('ANN index built')


def search_index(userid):
    a = AnnoyIndex(512, 'angular')
    with open('vectors/index_map.json', 'r') as file:
        index_map = json.load(file)

    user_index = list(index_map.keys())[list(index_map.values()).index(userid + '.npy')]
    a.load('vectors/index.ann')
